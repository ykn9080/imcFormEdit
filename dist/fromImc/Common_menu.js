...
exports.menuMy = menuMy;
exports.permissionarray = permissionarray;
...
var _Common_data = require("./Common_data");
...
const gb = () => {
  const gg = (0, _reactRedux.useSelector)(state => state.global);
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, "gg");
};

function menuMy(menu, menutype) {
  if (typeof menutype == "undefined") menutype = gb.menutoggle;
  let sl, grp;
  let storename = "imctable"; //if (menutype == "template")
  //    storename = 'imclist';
  // menutype = menutypereturn();

  let mm = (0, _Common_data.selectimc)(storename, menutype + menu);
  ...
  {
    let my = (0, _core.getlogin)();

    if (my != "" && gb.menutoggle != "open") {
      sl = [];
      if (my.hasOwnProperty("slevel")) sl = permissionarray(my.slevel);
      grp = permissionarray(my.group);
      (0, _jquery.default)(mm).each(function (i, k) {
        let chk = false;

        if (k.hasOwnProperty("permission") && k.permission != "") {
          let list = k.permission.split(",");
          (0, _jquery.default)(list).each(function (a, b) {
            if (a == 0 && _jquery.default.inArray(b, grp) == -1) chk = true;
            if (a == 1 && sl.length > 0 && _jquery.default.inArray(b, sl) == -1) chk = true;
          });
