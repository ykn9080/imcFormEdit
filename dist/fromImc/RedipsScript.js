...
var _Common_make = require("./Common_make.js");

var _Common_data = require("./Common_data.js");

var _Common_archive = require("./Common_archive.js");

var _core = require("./core.js");

var _Common_menu = require("./Common_menu.js");

var _Controls_jqgrid = require("./Controls_jqgrid.js");

var _Controls_jstree = require("./Controls_jstree.js");

var _Controls_googlechart = require("./Controls_googlechart.js");

var _Controls_fullCalendar = require("./Controls_fullCalendar.js");

var _Controls_content = require("./Controls_content.js");

var _Controls_map = require("./Controls_map.js");

var _Controls_pivot = require("./Controls_pivot.js");

var _Controls_rstat = require("./Controls_rstat.js");

var _Controls_form = require("./Controls_form.js");

var _Controls_xtra = require("./Controls_xtra.js");
...
var ctrtxt = ['Grid', 'Tree', 'Chart', 'Calendar', 'Content', 'Map', 'ifrm', 'PivotTable', 'R', 'Form', 'Process', 'Otherpage', 'Html']; // var menuid = "",
...
redips.configuration = function () {
  ...
  redips.component = '<table class="nolayout cHeader"><tr><td class="hLeft" onclick="redips.details(this)">+</td><td class="hTitle">|</td><td class="hRight" onclick="redips.divDelete(this)">x</td></tr></table>' + '<div class="' + redips.cDetails + '">|</div>';
...
redips.init = function () {
  ...
  var rt = REDIPS.table,
      rd = REDIPS.drag; // configuration

  redips.configuration(); // attach onmousedown event handler to tblEditor

  rt.onmousedown(redips.tableEditor, true); // show cellIndex (it is nice for debugging)
  //rt.cell_index(true);
  // selected cell background color

  rt.color.cell = redips.markedColor; // disable marking not empty table cells

  rt.mark_nonempty = false; // create XMLHttp request object
  //redips.request = redips.initXMLHttpClient();
  // initialize REDIPS.drag library

  rd.init(); // set drop mode as "single" - DIV element can be dropped only to the empty cells
  //rd.dropMode = 'single';
  ...
  rd.event.moved = function (targetCell) {
    if (targetCell) rd.obj.style.width = '150px'; //else {
  ...
  rd.event.droppedBefore = function (targetCell) {
    rd.obj.style.width = '100%'; // set new width to the dropped DIV element

    var width = targetCell.offsetWidth; // set width and reset height value

    rd.obj.style.width = width - 10 + 'px';
    rd.obj.style.height = ''; //edit½Ã ¹ß»ýÇÏ´Â ÀÏ·Ã¹øÈ£ÀÌ½´ÇØ°á
    //move½Ã¿¡ Àû¿ë¾ÈµÇ°ÔÇÔ

    var pos = rd.getPosition();

    if (pos[3] == 0) {
      //apply only to new insert
      var cnt = 0;
      var maxcnt = 0;
      ...
      for (var i = 0; i < divs.length; i++) {
        ...
        var div = divs[i]; // filter only components

        if (div.className.indexOf('drag') > -1) {
          if (rd.obj.id.substring(0, 1) == div.id.substring(0, 1)) cnt++; //prevent error when delete rows in the middle

          if (parseInt(div.id.substring(2)) > maxcnt) maxcnt = parseInt(div.id.substring(2));
        }
      }
  ...
  rd.event.dropped = function (targetCell) {
    ...
    var pos = rd.getPosition(); // define source table

    st = rd.findParent('TABLE', rd.td.source); // if source table is table editor then expand DIV element
    ...
    if (redips.components === st.id) {
      ...
      id = rd.obj.id.substring(0, 3); //          rd.obj.style.borderColor = 'white';
  ...
  rd.event.deleted = function () {
    ...
    (0, _Common_data.updateimctable)(menuid, subid, rd.obj.id, "");
    (0, _Common_data.deleteimc)("imctable", menutoggle + "control", "dvid", rd.obj.id);
    (0, _Common_data.jsonDelAjax)("imctable", menutoggle + "control", "dvid", rd.obj.id);
    console.log('deleted');
  };
}; //#region display layout


exports.initDisplay = initDisplay = function initDisplay(id, subdata, insertdv) {
  (0, _Common_make.clearinserted)();
  if (typeof subdata == "undefined" | subdata == "") subdata = selectimctable(menuid, subid);else if (typeof subdata != "object") subdata = JSON.parse(subdata);
  if (subdata == "") remoteimctable();
  ...
  if ($("#tableinsert").length == 0) {
    if ($("#dvContent123").length == 0) var parent = $($("form")[0]);else parent = $("#dvContent123");
  ...
  tb.append(table);
  tb.find("table").first().find("td").css({
    padding: "0 3px 3px 0"
  });
  injectObject2(tb);
  if ($("#resize1").length == 0) (0, _core.documentreadyInsert)("resize1", "$(window).resize(function () {refreshLayout();});");
  refreshLayout();
  tb.css({
    padding: "0 5px 0 5px"
  });
...
displayLayout = function displayLayout(id, data, wth) {
  $("#main-menu").show();
  var table = $("<table/>");
  ...
  $('#main-top>span').show(); //if mobile

  if (window.matchMedia("only screen and (max-width: 760px)").matches | (0, _core.isMobile)()) {
    $("form").removeAttr("style");
    $("body").removeAttr("style"); //.css({ width: $(window).width() })

    $('#main-top>span').hide();
    if ((0, _core.isMobile)()) var table = $("<div/>");
    $.each(data, function (rowIndex, r) {
      $.each(r, function (colIndex, c) {
        if (c.dv) $.each(c.dv, function (dvIndex, d) {
          var dv = $("<div />");
          dv.css({
            "padding": "0 5px 5px 0",
            "margin": "5px 0 5px 0"
    ...
    if (checkcolspan(data)) table = withcolspan(table, data);else table = withoutcolspan(table, data);
  }
  ...
  function withcolspan(table, data) {
    $.each(data, function (rowIndex, r) {
      ...
      $.each(r, function (colIndex, c) {
        ...
        } else width = unitwidth;

        if (c.rowspan) cell.attr("rowspan", c.rowspan);
        if (c.id) cell.attr("id", c.id);
        if (c.clas) cell.attr("class", c.clas); //style

        var sty = "",
            wpct = width;
        if (widthpct(wth).length > 0) wpct = widthpct(wth)[colIndex];
        sty = "width:" + wpct + "%;"; // sty = "width:" + realwth + "px;";
        // if (c.style) sty += c.style;

        if (sty != "" && sty.indexOf("vertical-align") == -1) sty += "vertical-align:top;";
        cell.attr("style", sty);
        if (c.dv) $.each(c.dv, function (dvIndex, d) {
          var dv = $("<div />");
          if (colIndex < r.length - 1 && rowIndex < (data.length - 1) / colnum) dv.css({
            "padding": "0 5px 5px 0"
          });
          dv.attr("id", d.id);
  ...
  function withoutcolspan(table, data) {
    var row = $("<tr/>");
    if (data != "") $.each(data, function (rowIndex, r) {
      //ttl column num
      var colnum = r.length;
      $.each(r, function (colIndex, c) {
        if (c.colspan) colnum += parseInt(c.colspan) - 1;
  ...
  function checkcolspan(data) {
    ...
    if (data != "") $.each(data, function (rowIndex, r) {
      $.each(r, function (colIndex, c) {
        if (c.hasOwnProperty("colspan") && parseInt(c.colspan) > 1) rtn = true;
      });
    });
    return rtn;
  } //width to pct
...
checksubmenu = function checksubmenu() {
...
async function injectObject2(dvid) {
  if (typeof dvid == "undefined") dvid = $("#tableinsert"); //var list = ["jqgrid", "select", "jstree", "googlechart", "fullcalendar", "content", "map", "ifrm", "pivot", "form","process","rstat","otherpage","html"];
  ...
  $.each(ctrarr, function (i, k) {
    $.each(dvid.find("." + k), function (index, data) {
      var id = $(data).attr("id"); //ctrarr.push({id:id,ctrclass:k});
      ...
      if (typeof ctrdata != "undefined") {
        ...
        if (!reload && dtexist) {
          ...
          var set = (0, _Common_data.makeDatabaseParamlist)(ctrdata.data);
  ...
  var insertDatalist = (controljson, datalist) => {
    ...
    if (controljson.hasOwnProperty("data")) {
      if (controljson.data.hasOwnProperty("querylist")) $.each(controljson.data.querylist, (i, k) => {
        if (k.sqlcommand == "select") k.datalist = datalist;
      });else controljson.data.datalist = datalist;
    }
  ...
  if (dtarray != "") {
    $.each(dtarray, (i, k) => {
      ...
      (0, _Common_data.updateimctablelocal)(menutoggle + "control", k.ctrid, controljson);
  ...
  setTimeout(function () {
    $(delayedhide).each(function (i, k) {
      if ($("#" + k).parent().hasClass("panel-body")) $("#" + k).closest(".panel").hide();else $("#" + k).hide();
    }); // $.each(dtarray, (i, k) => {
    //     initbyctr(k.ctrtype, k.ctrid);
    //     console.log("initbyctr", k.ctrtype,k.ctrid)
    // });
...
function initbyctr(ctrtype, id) {
  switch (ctrtype) {
    case "jqgrid":
      (0, _Controls_jqgrid.jqgridInit)(id);
      break;
    ...
    case "jstree":
      (0, _Controls_jstree.jstreeInit)(id);
      break;

    case "googlechart":
      (0, _Controls_googlechart.googlechartInit)(id);
      break;

    case "fullcalendar":
      (0, _Controls_fullCalendar.fullCalendarInit)(id);
      break;

    case "content":
      (0, _Controls_content.contentInit)(id);
      break;

    case "map":
      (0, _Controls_map.mapInit)(id);
      break;

    case "ifrm":
      (0, _Controls_xtra.iframeInit)(id);
      break;

    case "pivot":
      (0, _Controls_pivot.pivotInit)(id);
      break;

    case "rstat":
      (0, _Controls_rstat.rstatInit)(id);
      break;

    case "form":
      (0, _Controls_form.formInit)(id);
      break;

    case "process":
      (0, _Controls_xtra.processInit)(id);
      break;

    case "otherpage":
      (0, _Controls_xtra.otherpageInit)(id);
      break;

    case "html":
      (0, _Controls_xtra.htmlInit)(id);
      break;
  }
...
var isReload = data => {
  var interval = "",
      updated = "";
  if (data.hasOwnProperty("interval")) interval = data.interval;else if (data.hasOwnProperty("querylist")) {
  ...
  return (0, _Common_data.intervalcheck)(interval, updated); //in common_data
}; //check datalist exists in imctable in localstorage


var isDatalist = data => {
  var dtexist = false;
  if (data.hasOwnProperty("datalist")) dtexist = true;else if (data.hasOwnProperty("querylist")) {
    $(data.querylist).each((i, k) => {
...
window.initEdit = function () {
  ...
  if (imclist == null) localStorageinit({
    "include": ["imclist"]
  });
  var nodename = selectimctable(menuid, subid, "");
  if (typeof nodename != "undefined") nodename = nodename.text;else nodename = "";
  if ($("#tableinsert").length == 0) $(" <div id='tableinsert' class='layout' ></div>").appendTo($($("form")[0]));
  $(".layout").empty();
  $(".layout").append(_editLayout(menuid, subid));
  $("#tblEditor").colResizable({
    disable: true
  });
  tableResize($("#tblEditor"));
  fitwidth(); //makecontextmenu();
  //submenu tree
  ...
  (0, _Common_menu.menuTree)("imcMenu", options); //menutoggle = "admin";
  //append topmenu

  var topm = (0, _Common_menu.menuMy)("menu"),
      list = [],
      tlist = "";
  ; // selectimc("imctable", "menu")
  ...
  $('[data-toggle="tooltip"]').tooltip(); //    jscriptInsert("theme", "/js2/jquery-themeroller/jquery.themeswitcher.min.js");
  // $("#tableinsert").css({ "margin-top": "10px" });

  funStop(); //menuBreadcrumbs($($(this)[0]));
  //$("#dvName").empty();
  //var title = "Page Build";
  //$("#dvName").append($("<label style='font-size:2em;padding-top:10px;'>" + title + "</label>"));
};

_editLayout = function editLayout() {
  console.log('menuid,subid:', menuid, subid);
  var data1 = selectimctable(menuid, subid),
      data;
  var drag, left, right, table1, tr1, td1, dv1, menu; //table1 ÁÂÃø droppable menu

  table1 = $("<table/>");
  table1.attr("id", "table1");
  table1.append("<colgroup><col width='100'/></colgroup>");
  var tb1 = [["a", "drag clone orange1 jqgrid", "Grid"], ["b", "drag clone green1 select", "Select"], ["c", "drag clone blue1 jstree", "Tree"], ["d", "drag clone yellow1 googlechart", "Chart"], ["e", "drag clone yellow1 fullcalendar", "Calendar"], ["g", "drag clone green1 map", "Map"], ["h", "drag clone blue1 ifrm", "ifrm"], ["f", "drag clone yellow1 content", "Content"], ["i", "drag clone yellow1 pivot", "PivotTable"], ["j", "drag clone yellow1 form", "Form"], ["k", "drag clone yellow1 otherpage", "Otherpage"], ["l", "drag clone yellow1 html", "HTML"], ["m", "drag clone yellow1 rstat", "R"], ["n", "drag clone yellow1 process", "Process"]];
  ...
  td1.text("Trash");
  tr1.append(td1);
  table1.append(tr1); //left

  left = $("<div/>");
  left.attr("id", "left");
  left.append($("<div id='dvmenutoggle'></div>")); //menu dropdown
  ...
  dv1.attr("id", "imcMenu");
  left.append(dv1);
  left.append(table1); //right

  right = $("<div/>");
  right.attr("id", "right"); //right-buttontop
  ...
  [["R+", "reSize('dis');REDIPS.table.row('tblEditor','insert');reSize('en')"], ["R-", "reSize('dis');delRow();reSize('en')"], ["C+", "reSize('dis');REDIPS.table.column('tblEditor','insert');reSize('en')"], ["C-", "reSize('dis');delCol();reSize('en')"]];
  $.each(list, function (i, k) {
    var inp = $("<span/>");
    td1 = $("<td style='padding:0 0 5px 0;width:25px;'/>"); //td1.attr("class", "mark");
    //inp.attr("type", "button");

    inp.attr("class", "badge badge-info cursorbtn marginright"); //inp.attr("value", k[0]);

    inp.text(k[0]);
    inp.attr("onclick", k[1]);
    td1.append(inp);
    inp.wrap("<h4/>");
    tr1.append(td1); //inp.button();
  });
  ...
  otherpage.on("click", function (e) {
    var dataset = menutreedata('id', false, {
      level: "submenu",
      hascontrol: true
    });
    var mlistel = $("<div class='form-inline'><span>Menu List: </span> " + makeCtr(["select", "All,all;Current,current", "selmlist", "", ""]) + "</div>");
    reloadmenutree('id', false, {
      dataset: dataset,
      plugin: [],
      mlistel: mlistel,
      callback: _editLayout.process
    });
  });

  _editLayout.process = function () {
    ...
    swal({
      ...
      if (isConfirm) {
        ...
        var nctlist = [];
        console.log(curpage, copypage); //submenu replace with copy submenu
        ...
        $(sublist).each(function (a, b) {
          if (b.subid == subid) sublist.splice(a, 1, copypage);
        ...
        (0, _Common_data.remoteimcupdate)("imctable");
        setTimeout(function () {
          $("#ahbuild").click();
        }, 500);
      }
    ...
    function replaceid(ctr1) {
      ...
      if (ctr1.hasOwnProperty("dvid")) {
        ctr1.dvid = ctr1.dvid.substring(0, 2) + (0, _core.idMake)();
      }
  ...
  edit.on("click", function (e) {
    ...
    setTimeout(function () {
      ...
      $("#dvTitle").append($("<div style='float:left;margin-left:10px;'>" + "<input type='button' onclick=\"$('#ahbuild').click();$(this).parent().remove();\" value='<< Back'/></div>"));
  ...
  table1.attr("style", "table-layout:fixed;width:100%;"); //var data = selectimc('imctable', convertId('layout_0'));
  ...
  if (data1 != "" && typeof data1 != "undefined") {
    ...
    if (!data1.hasOwnProperty("table") || data1.table.length == 0) data = [[{
      "id": 0,
      "style": ":"
    }, {
      "id": 1,
      ...
      "id": 2,
      "style": ":"
    }]];else data = data1.table; //else data=[[{"id":0,"style":"vertical-align:top;:","dv":[]}]];
    ...
    $.each(data, function (rowIndex, r) {
      ...
      $.each(r, function (colIndex, c) {
        ...
        if (rowIndex == 0) {
          var th = $("<th>&nbsp;</th>");
          if (wth != "") th.css("width", widthpct(wth)[colIndex] + "%"); // if (c.colspan) th.attr("colspan", c.colspan);
        ...
        if (colIndex == 0) cls += " left";
        if (colIndex == r.length - 1) cls += " right";
        cell.attr("class", cls);
        var sty = "";
        if (typeof c.style != "undefined") sty = c.style;
        if (sty == "") sty = "vertical-align:top;";else if (sty.indexOf("vertical-align") == -1) sty += ";vertical-align:top;";
        cell.attr("style", sty);
        ...
        var dv = $("<div/>");
        dv.attr("style", "text-align:right;height:22px;"); //dv.attr("class", "showhide");
        ...
        if (c.dv) $.each(c.dv, function (dvIndex, d) {
          ...
          if (d.clas) {
            ...
            $(ctrarr).each(function (a, b) {
              if (d.clas.indexOf(b) > -1) dv.text(ctrtxt[a]);
            });
          }
  ...
  dv.append(table1);
  right.append(dv);
  (0, _Common_make.rowSortable)(table1); //right-buttonbottom
  ...
  list = [["inE", "Apply", "funLoading(true);editSave(true);" + "setTimeout(()=>{menuMain();setTimeout(()=>{$(\"#main-menu a[subid='" + subid + "']\").click();},700);},2500);"], ["inL", "Save", "funLoading(true);editSave(true);initEdit();redips.init();"], ["inS", "Cancel", "funLoading(true);initDisplay('',selectimctable(menuid,subid))"]];
  ...
  $.each(list, function (i, k) {
    ...
    inp.attr("value", k[1]); //inp.text(k[1]);
  ...
  right.find("div").last().prepend((0, _Common_archive.archiveattach)(aset));
  (0, _core.styleInsert)("content", ".content{margin:3px 0 3px 0 !important;}.clone{width:155px !important;}");
...
function findimctable(val, opt) {
  ...
  $(["", "open", "admin"]).each(function (a, b) {
    ...
    var spage = (0, _Common_menu.menuMy)("submenu", b).filter(function (v) {
      return v.subid == val;
    });
    var control = (0, _Common_menu.menuMy)("control", b).filter(function (v) {
      return v.dvid == val;
    });
  ...
  if (typeof opt != "undefined") rtn = type;
  return rtn;
}
...
window.delRow = function () {
  ...
  $(tds).each(function (i, k) {
    if ($(k).children().length > 1) emptyrow = false;
  });
  if (emptyrow) REDIPS.table.row('tblEditor', 'delete');else swal("Last row is not empty!!");
};
...
window.delCol = function () {
  ...
  $(lasttds).each(function (i, k) {
    if ($(k).children().length > 1) empty = false;
  });
  if (empty) REDIPS.table.column('tblEditor', 'delete');else swal("Last column is not empty!!");
};
...
window.reSize = function (type) {
  ...
  switch (type) {
    ...
    case "en":
      ...
      $("#tblEditor tr").each(function (i, row) {
        $(row).children().each(function (j, cell) {
          if ($(this).children().length == 0) {
            ...
            if (i == 0) newwidth += "," + $(this).css("width");
          }
        });
      ...
      var tr = $('<tr/>').prependTo(thead); //var tr = $('<tr/>').prependTo("#tblEditor > tbody");

      $.each(row, function (colIndex, c) {
        var tth = $('<th>&nbsp;</th>');
        if (wtharr.length > 0) tth.css("width", wtharr[colIndex]);
        tr.append(tth);
      });
      tableResize(tb);
      break;
...
window.editObject = function (obj) {
  ...
  window.editObject.process = function (obj) {
    reset(); // if (menutoggle == "admin") menutoggle = "";

    datartnctr = "";
    var clsarr = obj.class.split(" "),
        cls;
    $(clsarr).each(function (i, k) {
      if ($.inArray(k, ctrarr) > -1) cls = k;
    });

    switch (cls) {
      case "jqgrid":
        (0, _Controls_jqgrid.jqEdit)(obj.id);
        break;
      ...
      case "jstree":
        (0, _Controls_jstree.jstreeEdit)(obj.id);
        break;

      case "googlechart":
        (0, _Controls_googlechart.googlechartEdit)(obj.id);
        break;

      case "fullcalendar":
        (0, _Controls_fullCalendar.fullCalendarEdit)(obj.id);
        break;

      case "content":
        (0, _Controls_content.contentEdit)(obj.id);
        break;

      case "map":
        (0, _Controls_map.mapEdit)(obj.id);
        break;

      case "ifrm":
        (0, _Controls_xtra.iframeEdit)(obj.id);
        break;

      case "pivot":
        (0, _Controls_pivot.pivotEdit)(obj.id);
        break;

      case "rstat":
        (0, _Controls_rstat.rstatEdit)(obj.id);
        break;

      case "form":
        (0, _Controls_form.formEdit)(obj.id);
        break;

      case "process":
        (0, _Controls_xtra.processEdit)(obj.id);
        break;

      case "otherpage":
        (0, _Controls_xtra.otherpageEdit)(obj.id);
        break;

      case "html":
        (0, _Controls_xtra.htmlEdit)(obj.id);
        break;
    }

    $("#tab-Contain>div>.ui-tabs-panel").css({
  ...
  if (typeof imc != "undefined" && imc != "" && imc.hasOwnProperty("data")) {
    ...
    $("#spdlist").text(JSON.stringify((0, _Common_data.datalistreturn)(imc.data))); //datacode = datacodereturn(imc.data);
    ...
    window.editObject.process(obj); // if (datacode != "")
...
scanTable = function scanTable() {
  ...
  $('#tblEditor').find('tr').each(function (i, tr) {
    ...
    $(tr).children().each(function (j, td) {
      ...
      if ($(td).prop("tagName") == "TD") {
        if ($(td).find('SPAN').attr("class") != "rowTool") {
          tdd = {};
          tdd.id = j;
          if ($(td).hasClass("hideobj")) tdd.display = "none";
          if ($(td).attr("colspan") != "undefined") tdd.colspan = $(td).attr("colspan");
          if ($(td).attr("rowspan") != "undefined") tdd.rowspan = $(td).attr("rowspan");
          ...
          if (sty != "") {
            ...
            $.each(sty, function (i, k) {
              var sp = k.split(":");
              if (sp[0] != "width") newsty.push($.trim(sp[0] + ":" + $.trim(sp[1])));
            });
          ...
          trr.push(tdd);
          dvv = [];
          $(td).children().each(function (k, dv) {
            //div
            if (k > 0) {
              objj = {};
              var txt = "";
              ...
              $(ctrarr).each(function (a, b) {
                if (objj.clas.indexOf(b) > -1) objj.txt = ctrtxt[a];
              }); //if ($(dv).text() != "")
              //    objj.txt = $(dv).text();
    ...
    if (trr.length > 0) rtn.push(trr);
  });
  return rtn;
};

scanObject = function scanObject(dv) {
...
window.editSave = remote => {
  ...
  if (!exist) {
    ...
    set.text = text;
    set.parent = menuid;
    if (menutoggle == "template") set.templatetype = templatetype;
    set.table = scanTable();
  ...
  if (menutoggle == "template") {
    ...
    localStorage.setItem("imctable", JSON.stringify(imctb));
    if (remote) (0, _Common_data.remoteimcupdate)("imctable");
  ...
  function addarchive(set) {
    ...
    if (archivesave) set.archive = ardt;
    return set;
  }
  ...
  function templatesave(set) {
    ...
    if (set.hasOwnProperty("archive")) {
      ...
      set.text = ardt.name;
      set.desc = ardt.desc;
      set.templatetype = "current"; //clean id for template
    } //scan controls in table field and save them in templaecontrol


    var ctrlist = [];
    if (set.hasOwnProperty("table")) var tblist = set.table; //control copy & insert to imctable local & server
...
var colwidth = ''; //resizable column width

tableResize = function tableResize(tb) {
  var onSampleResized = function onSampleResized(e) {
    ...
    (0, _Common_data.updateimctable)(menuid, subid, "", tb); //fit control width to each td
...
editcell = function editcell(cellid) {
  var rc = cellid.split(',');
  var dv = $("<div/>"),
      sty = "",
      rspan = "",
  ...
  dv.attr("id", "dvtbedit");
  var data = [[makeCtr(["span", "Type",,,]), makeCtr(["span", "Value",,,])] //headers
  , ["vertical-align", makeCtr(["select", "top,top;middle,middle;bottom,bottom", "selVt", "inp"])], ["text-align", makeCtr(["select", "left,left;center,center;right,right", "selTa", "inp"])], ["margin", makeCtr(["input", mgn, "inpmgn", "inp"])], ["padding", makeCtr(["input", pdg, "inppdg", "inp"])], ["display", makeCtr(["select", "block;none", "seldisplay", "inp"])], ["colspan", makeCtr(["input", cspan, "", "inp"])], ["rowspan", makeCtr(["input", rspan, "", "inp"])]];
...
editcellsave = function editcellsave(cellid) {
  var rc = cellid.split(',');
  var rdt = ""; //rdt["width"] = $("#inpWidth").val();
    ...
    if (i == rc[0]) {
      $.each(k, function (j, l) {
        if (l.id == rc[1]) {
          ...
          if (disp != "" && disp != "display") l.display = disp;else delete l.display;
        }
      });
    }
  }); //table style change
  ...
  (0, _Common_make.rowSortable)($("#tblEditor"));
  console.log(subdt);
  (0, _Common_data.updateimctable)(menuid, subid, "", subdt);
...
showContent = function showContent() {
  ...
  message = document.getElementById('message'); // show block content

  message.innerHTML = 'td1 = ' + td1 + '<br>' + 'td2 = ' + td2 + '<br>' + 'td3 = ' + td3 + '<br>' + 'td4 = ' + td4;
...
getContent = function getContent(id) {
  var td = document.getElementById(id),
      content = '',
      cn,
      i; // TD can contain many DIV elements

  for (i = 0; i < td.childNodes.length; i++) {
    // set reference to the child node
    cn = td.childNodes[i]; // childNode should be DIV with containing "drag" class name
  ...
  content = content.substring(0, content.length - 1); // return result
...
redips.initXMLHttpClient = function () {
  var XMLHTTP_IDS,
      xmlhttp,
      success = false,
      i; // Mozilla/Chrome/Safari/IE7/IE8 (normal browsers)
...
redips.getComponent = function (obj, id) {// open asynchronus request
...
redips.divDelete = function (el) {
  var div = REDIPS.drag.findParent('DIV', el),
      // set reference to the DIV element
  rcell = el.parentNode.cells[1],
      // set reference to the right cell of DIV element header
  name = rcell.innerText || rcell.textContent; // set name in a cross-browser manner
  // set name to lower case

  name = name.toLowerCase(); // confirm deletion
...
redips.details = function (el, type) {
  var divDrag = REDIPS.drag.findParent('DIV', el),
      // find parent DIV element
  tbl = divDrag.childNodes[0],
      // first child node is table
  div = divDrag.childNodes[1],
      // second child node is hidden DIV (with containing component details)
  ...
  if (type === undefined || type === 'show') {
    ...
    div.style.zIndex = 999; // element with position: absolute is taken out of the normal flow of the page and positioned at the desired coordinates relative to its containing block
    // http://www.quirksmode.org/css/position.html

    div.style.position = 'absolute'; // http://foohack.com/2007/10/top-5-css-mistakes/ (how z-index works)
    ...
    div.style.zIndex = -1;
    div.style.position = ''; // return opacity value (if opacity is removed from style.css then this line should be removed as well)
...
redips.save = function () {
  ...
  var tblEditor = document.getElementById(redips.tableEditor),
      ...
      frm,
      // form reference inside component (it should be only one form)
  JSONobj = [],
      // prepare JSON object
  json,
      // json converted to the string
  component,
      // component object
  div,
      // current DIV element
  ...
  for (i = 0; i < divs.length; i++) {
    // set current DIV element
    div = divs[i]; // filter only components

    if (div.className.indexOf('drag') > -1) {
      // initialize component object
      component = {}; // component id (only first three characters because cloned element will have addition in id)

      component.id = div.id.substring(0, 3); // component position
      ...
      frm = div.getElementsByTagName('FORM')[0]; // call method to scan component form and return all form elements with their values

      component.form = redips.form2obj(frm); // push values for DIV element as Array to the Array
...
redips.form2obj = function (frm) {
  ...
  for (i = 0; i < frm.elements.length; i++) {
    ...
    name = frm.elements[i].name; // switch on element type
...
redips.merge = function () {
  ...
  REDIPS.table.merge('h', false); // then merge cells vertically and clear cells (second param is true by default)
...
redips.rowInsert = function (el) {
  ...
  top_row,
      // cells reference in top row of the table editor
  nr,
      // new table row
  lc; // last cell in newly inserted row
  // set reference to the top row cells

  top_row = row.parentNode.rows[0].cells; // insert table row

  nr = REDIPS.table.row(redips.tableEditor, 'insert', row.rowIndex + 1);
  nr.setAttribute("style", "vertical-align:top;"); // define last cell in newly inserted table row

  lc = nr.cells[nr.cells.length - 1]; // copy last cell content from the top row to the last cell of the newly inserted row

  lc.innerHTML = top_row[top_row.length - 1].innerHTML; // ignore last cell (attached onmousedown event listener will be removed)
...
redips.rowDelete = function (el) {
  // find source row (skip inner row)
  var row = REDIPS.drag.findParent('TR', el); // confirm deletion
...
prefixFind = function prefixFind(obj) {
  var set = {};
...
prefixFind1 = function prefixFind1(id) {
